...
<body data-print-page-size="10">
	<div class="print-wrapper print-page-break fs-xs" v-for="(page, index) in data.sfpzmxPages">
		<div>
			<h4 class="title" style="padding-top:1em;"><span>押金退还</span></h4> <!-- 不同设备可能需要调整padding-top -->
			...
			<div class="wrapper">
				...
				<div class="row">
					...
					<div class="col-3">退款方式：{{data.sfpz.thjsfsmc}}</div>
			...
			<div class="table-wrapper f-weight-bold border-h mt-2"> <!-- border, border-h -->
				<table class="table border-h-tbody-none" border="0" cellspacing="0"> <!-- border, border-h, border-h-tbody-none -->
				    <thead class="thead-light">
				        <tr>
				        	<th class="text-left" style="width:auto;">押金类型</th>
				        	<th class="text-left" style="width:10em;">收款编号</th>
				            <th class="text-center" style="width:13em;">退还时间</th>
				            <th class="text-right" style="width:10em;">金额</th>
				        </tr>
				    </thead>
					<tbody>
						<tr v-for="sfpzmx in page.sfpzmxs">
							<td>{{sfpzmx.yjlxmc}}</td>
							<td class="f-family-regular">{{sfpzmx.skpjbh?sfpzmx.skpjbh:'-'}}</td>
							<td class="text-center f-family-regular">{{sfpzmx.tksj}}</td>
							<td class="text-right"><span class="price f-family-regular" v-show="typeof sfpzmx.sqje==='number'">{{typeof sfpzmx.sqje==='number'?sfpzmx.sqje.toFixed(2):'-'}}</span></td>
							</tr>
