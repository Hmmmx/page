...
function getBaxc(opt){
	let wrap = $('.page-body .baxc-list-wrapper .baxc-list-body');
	...
	if (opt.reset) {
		$('.page-body .baxc-list-wrapper .baxc-list-body .baxc').remove();
	...
	if (opt.init) {
		let _click = async function(){
	    	...
	    	if ((!more.data('last-checking') || now-more.data('last-checking')>400) && (more.data('loading') != 'Y' && more.data('has-more') != 'N')) {
		    	...
		    	data.pageSize = 50;
		    	data.page = Math.floor($('.page-body .baxc-list-wrapper .baxc-list-body .baxc').length/data.pageSize);
		    	let res = await p_getBaxc(data);
		    	
		    	if (res.code == '0') renderBaxcList(res.data);
		    	else CxCtrl.alert(res.message);
...
function p_getBaxc(data){
	return new Promise(function(resolve, reject){
		CxMisc.ajaxwx({
		    url: CxMisc.finalizeUrl('/gz/wyhzs/baxc/getXcmx'),
		    type: "GET",
		    data: data,
		    success: function(res, ts) {
		    	if (res.code == "0")
...
function renderBaxcList(data) {
	let wrap = $('.page-body .baxc-list-wrapper .baxc-list-body');
	for (let i=0; i<data.length; i++) {
		let item = data[i], xcsm = item.xcsm?('巡查说明：'+item.xcsm):'无巡查说明', 
			xcsmClass=item.xcsm?' text-black-50':' text-black-50 d-none';
		wrap.append(`<div class="baxc" data-id="${item.baxcmxid}">
				<div class="mb-1">${CxMisc.escapeHtml(item.xcddbt)}</div>
	...
	wrap.find('button[data-cmd="show-mx"]').click(function(){ showBaxcMx(this); });
}
...
function showBaxcMx(el) {
	let target = $(el), wrap = target.closest('.baxc');
		
	let imgs = [], imgsWrapper = [], item = target.closest('.baxc').data('json');
	if (item.xctpdz1)
...
let gHyxx = null;
$(function(){
	let baxcid = CxMisc.qs.get('baxcid');
	if (baxcid) {
		window.location.href = CxMisc.finalizeUrl4Wx('/gz/wyhzs/baxc/add?baxcid='+baxcid);
	} else {
		prepare();
		bind();
	}
