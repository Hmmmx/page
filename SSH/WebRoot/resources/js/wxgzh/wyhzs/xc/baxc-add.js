async function prepare(){
	...
	if (hyxx.code) {
		...
		if (baxcid) {
			let xcdd = await p_getXcdd(baxcid);
			if (xcdd) {
				$('.page-body [data-f-name=sqmc]').text(xcdd.sqmc);
				$('.page-body [data-f-name=xcdd]').text(xcdd.xcddbt);
				if (xcdd.bz) $('.page-body [data-f-name=bz]').text(xcdd.bz).closest('.cx-pair').removeClass('d-none');
			} else {
				document.querySelector('.page-action-bar button[data-cmd=save]').disabled = true;
				CxCtrl.alert('找不到该巡查地点信息，可能已失效，请与物业确认');
			}
		} else document.querySelector('.page-action-bar button[data-cmd=save]').disabled = true;
	}
}

function p_getXcdd(baxcid) {
	return new Promise(function(resolve, reject){
		CxMisc.ajaxwx({
		    url: CxMisc.finalizeUrl('/gz/wyhzs/baxc/getXcdd'),
		    type: "GET",
		    data: {baxcid},
		    success: function(res, ts) {
		    	if (res.code == "0")
		    		...
		    		resolve({code:'failed', message: '获取巡查地点信息失败：' + res.message});
		    },
		    error: function(xhr, ts, err) {
		    	var msg = "[" + xhr.status + " : " + ts + "]";
		    	resolve({code:'error', message: '获取巡查地点信息失败：' + msg});
		    }
		});
	});
}
...
function submit(e) {
	...
	if (baxcid) {
		...
		if (CxMisc.validate(f)) {
			...
			CxMisc.ajaxwx({
			    url: CxMisc.finalizeUrl('/gz/wyhzs/baxc/addXcmx'),
			    type: "POST",
			    data: data,
			    beforeSend: function(xhr, cfg) {
		        	CxMisc.markAjaxStart($(el));
...
function bind(){
	$('.page-title .back').click(function(e){
		window.location.href = CxMisc.finalizeUrl4Wx('/gz/wyhzs/baxc');
	});
