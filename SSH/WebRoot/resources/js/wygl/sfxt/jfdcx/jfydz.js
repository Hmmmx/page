if (typeof window.SfCwbbJfydz === 'undefined') { // 可能多次载入运行，只在第一次时执行定义与赋值
	const cntrSelector = '#wgSfCwbbJfydzCntr';
	
	const mainTblClz = '.table-wgsf-cwbb-jfydz';
	const filterFrmSelector = '#wgSfCwbbJfydzCntr .filterbar>form[data-type=filter]';
	
	const loadingMask1 = '#wgSfCwbbJfydzCntr';
	
	const me = window.SfCwbbJfydz = {
		pager: null,
		filterData: null,
		...
		/*getJgSfxm: function(src) {
			let sqdm = src.getAttribute('data-sqdm'), el = src.form.sfxmdm;
		...
		dc: function(el) {
			...
			me.filterData = data;
			if (me.filterData) {
				$.fileDownload(CxMisc.finalizeUrl('/wygl/sfxt/jfdcx/dcjfyzd'), {
	                httpMethod : 'GET',
	                data : me.filterData,
