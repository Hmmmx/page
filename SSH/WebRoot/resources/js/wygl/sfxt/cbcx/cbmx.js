if (typeof window.SfWycxCbmx === 'undefined') { // 可能多次载入运行，只在第一次时执行定义与赋值
	const cntrSelector = '#wgSfWycxCbmxCntr';
	
	const mainTblClz = '.table-wgsf-wycx-cbmx';
	const filterFrmSelector = '#wgSfWycxCbmxCntr .filterbar>form[data-type=filter]';
	
	const loadingMask1 = '#wgSfWycxCbmxCntr';
	
	const me = window.SfWycxCbmx = {
		pager: null,
		filterData: null,
		...
		dc: function(el) {
			if (me.filterData) {
				$.fileDownload(CxMisc.finalizeUrl('/wygl/sfxt/cbcx/dcCbmx'), {
	                httpMethod : 'GET',
	                data : me.filterData,
		...
		filter: function(f) {
			...
			me.filterData = data;
			if (me.filterData.sqcbrqq && me.filterData.sqcbrqz && me.filterData.sqcbrqq>me.filterData.sqcbrqz) {
				CxMsg.info('上次抄表开始日期不能大于结束日期');
				return;
			}
			if (me.filterData.bqcbrqq && me.filterData.bqcbrqz && me.filterData.bqcbrqq>me.filterData.bqcbrqz) {
				CxMsg.info('上次抄表开始日期不能大于结束日期');
				return;
			}
			CxMisc.ajax({
			    url: CxMisc.finalizeUrl('/wygl/sfxt/cbcx/getCbmxCount'),
			    type: "GET",
			    data: me.filterData,
		...
		goto: function(page, pageSize) {
			CxMisc.ajax({
			    url: CxMisc.finalizeUrl('/wygl/sfxt/cbcx/getCbmx'),
			    type: "GET",
		...
		render: function(page, pageSize, data) {
			...
			for (let i=0; i<data.length; i++) {
				...
				tmp.append(`<tr>
						...
						<td>${item.yblxmc}</td>
						<td>${item.ybbh!==null?item.ybbh:''}</td>
						<td>${item.bl!==null?item.bl:''}</td>
						<td>${item.sqcbrq!==null?item.sqcbrq:''}</td>
						<td>${item.bqcbrq!==null?item.bqcbrq:''}</td>
						<td>${item.ghbj=='1'?'是':'否'}</td>
						<td>${item.jbyl!==null?item.jbyl:''}</td>
						<td>${item.sqds!==null?item.sqds:''}</td>
						<td>${item.bqds!==null?item.bqds:''}</td>
						<td>${item.bqyl!==null?item.bqyl:''}</td>
						<td>${item.sjyl!==null?item.sjyl:''}</td>
						<td>${item.czry!==null?item.czry:''}</td>
