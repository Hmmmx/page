if (typeof window.SfCwbbFysqtj === 'undefined') { // 可能多次载入运行，只在第一次时执行定义与赋值
	const cntrSelector = '#wgSfCwbbFysqtjCntr';
	
	const filterFrmSelector = '#wgSfCwbbFysqtjCntr .filterbar>form[data-type=filter]';
	
	const loadingMask1 = '#wgSfCwbbFysqtjCntr';
	
	const me = window.SfCwbbFysqtj = {
		pager: null,
		filterData: null,
		hzxmList: null,
		sums: null,
		...
		dc: function(el) {
			...
			if (me.filterData) {
				...
				$.fileDownload(CxMisc.finalizeUrl('/wygl/sfxt/fytj/dcFytj'), {
	                httpMethod : 'GET',
	                data : data1,
