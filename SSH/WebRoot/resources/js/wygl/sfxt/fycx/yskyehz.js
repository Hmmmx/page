if (typeof window.SfCwbbYskyehz === 'undefined') { // 可能多次载入运行，只在第一次时执行定义与赋值
	const cntrSelector = '#wgSfCwbbYskyehzCntr';
	
	const mainTblClz = '.table-wgsf-cwbb-ysk-yehz';
	const filterFrmSelector = '#wgSfCwbbYskyehzCntr .filterbar>form[data-type=filter]';
	
	const loadingMask1 = '#wgSfCwbbYskyehzCntr';
	
	const me = window.SfCwbbYskyehz = {
		pager: null,
		filterData: null,
		hzxmList: null,
		sums: null,
		...
		dc: function(el) {
			if (me.filterData) {
				...
				$.fileDownload(CxMisc.finalizeUrl('/wygl/sfxt/yskcx/dcYskyehz'), {
	                httpMethod : 'GET',
	                data : data,
		...
		filter: function(f) {
			...
			CxMisc.ajax({
			    url: CxMisc.finalizeUrl('/wygl/sfxt/yskcx/getYskyehzCount'),
			    type: "GET",
			    data: me.filterData,
			    beforeSend: function(xhr, cfg) {
		...
		goto: function(page, pageSize) {
			CxMisc.ajax({
			    url: CxMisc.finalizeUrl('/wygl/sfxt/yskcx/getYskyehz'),
			    type: "GET",
		...
		render: function(page, pageSize, data) {
			...
			for (let i=0; i<hzfcList.length; i++) {
				let hzfc = hzfcList[i], dqye = [], subTotal = 0, khmc = CxMisc.escapeHtml(hzfc.khmc), khmcTitle=hzfc.khmc&&hzfc.khmc.length>7?` title="${khmc}"`:'';
				for (let j=0; j<me.hzxmList.length; j++) {
					dqye.push(`<td>${hzfc.dqye[me.hzxmList[j].sfxmdm]?hzfc.dqye[me.hzxmList[j].sfxmdm].toFixed(2):'0.00'}</td>`);
					if (hzfc.dqye[me.hzxmList[j].sfxmdm]) subTotal += hzfc.dqye[me.hzxmList[j].sfxmdm];
				}
				dqye.push(`<td>${subTotal.toFixed(2)}</td>`);
				tmp.append(`<tr>
						<td class="td-indexer">
			...
			if (me.sums) { //如果有所有房产合计
				let sums = [], total = 0;
				for (let j=0; j<me.hzxmList.length; j++) {
					let sum = me.sums.dqye[me.hzxmList[j].sfxmdm] ? me.sums.dqye[me.hzxmList[j].sfxmdm] : 0;
