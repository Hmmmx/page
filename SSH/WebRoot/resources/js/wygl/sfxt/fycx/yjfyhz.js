if (typeof window.SfCwbbYjfyhz === 'undefined') { // 可能多次载入运行，只在第一次时执行定义与赋值
	const cntrSelector = '#wgSfCwbbYjfyhzCntr';
	
	const filterFrmSelector = '#wgSfCwbbYjfyhzCntr .filterbar>form[data-type=filter]';
	
	const loadingMask1 = '#wgSfCwbbYjfyhzCntr';
	
	const me = window.SfCwbbYjfyhz = {
		pager: null,
		filterData: null,
		hzxmList: null,
		sums: null,
		...
		dc: function(el) {
			...
			if (me.filterData) {
				...
				$.fileDownload(CxMisc.finalizeUrl('/wygl/sfxt/yjhz/dcYjhz'), {
	                httpMethod : 'GET',
	                data : data1,
