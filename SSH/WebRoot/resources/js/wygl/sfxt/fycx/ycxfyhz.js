if (typeof window.SfCwbbYcxfyhz === 'undefined') { // 可能多次载入运行，只在第一次时执行定义与赋值
	const cntrSelector = '#wgSfCwbbYcxfyhzCntr';
	
	const mainTblClz = '.table-wgsf-cwbb-ycxfyhz';
	const filterFrmSelector = '#wgSfCwbbYcxfyhzCntr .filterbar>form[data-type=filter]';
	
	const loadingMask1 = '#wgSfCwbbYcxfyhzCntr';
	
	const me = window.SfCwbbYcxfyhz = {
		pager: null,
		filterData: null,
		hzxmList: null,
		sums: null,
		...
		dc: function(el) {
			if (me.filterData) {
				...
				$.fileDownload(CxMisc.finalizeUrl('/wygl/sfxt/ycxcx/dcSfhz'), {
	                httpMethod : 'GET',
	                data : data,
		...
		filter: function(f) {
			...
			CxMisc.ajax({
			    url: CxMisc.finalizeUrl('/wygl/sfxt/ycxcx/getSfhzCount'),
			    type: "GET",
			    data: me.filterData,
			    beforeSend: function(xhr, cfg) {
		...
		goto: function(page, pageSize) {
			CxMisc.ajax({
			    url: CxMisc.finalizeUrl('/wygl/sfxt/ycxcx/getSfhz'),
			    type: "GET",
