if (typeof window.SfYdDxtz === 'undefined') { // 可能多次载入运行，只在第一次时执行定义与赋值
	const cntrSelector = '#wgSfYdDxtzCntr';
	const mbFrmSelector = '#wgSfYdDxtzMbFrm';
	const fsFrmSelector = '#wgSfYdDxtzFsFrm';
	
	const loadingMask1 = '#wgSfYdDxtzCntr';
	
	const me = window.SfYdDxtz = {
		bind: function() {
			me.refresh();
		...
		getMb: function(){
			CxMisc.ajax({
			    url: CxMisc.finalizeUrl('/wygl/sfxt/dxtz/getMbxx'),
			    type: "GET",
			    ...
			    success: function(res, ts) {
			    	if (res.code == "0") {
			    		...
			    		CxMsg.error('载入短信模板失败：' + res.message);
			    	}
			    },
			    error: function(xhr, ts, err) {
			    	var msg = "[" + xhr.status + " : " + ts + "]";
			    	CxMsg.error('载入短信模板失败：' + msg);
			    },
		...
		submitFs: function(el) {
			if (CxMisc.validate(el.form)) {
				...
				CxMisc.ajax({
				    url: CxMisc.finalizeUrl('/wygl/sfxt/dxtz/addDxtz'),
				    type: "POST",
				    data: data,
				    beforeSend: function(xhr, cfg) {
