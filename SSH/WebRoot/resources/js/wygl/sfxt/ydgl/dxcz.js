if (typeof window.SfYdDxcz === 'undefined') { // 可能多次载入运行，只在第一次时执行定义与赋值
	const cntrSelector = '#wgSfYdDxczCntr';
	const czModalSelector = '#wgSfYdDxczModalDtls';
	const czFrmSelector = '#wgSfYdDxczDtlsFrm';
	
	const mainTblClz = '.table-wgsf-yd-dxcz';
	const idPrefix = 'wgSfYdDxcz';
	const loadingMask1 = '#wgSfYdDxczCntr';
	
	const me = window.SfYdDxcz = {
		bind: function() {
			me.refresh(); 
		...
		fetch: function() {
			...
			CxMisc.ajax({
			    url: CxMisc.finalizeUrl('/wygl/sfxt/dxcz/getBgjl'),
			    type: "GET",
		...
		getSyts: function() {
			CxMisc.ajax({
			    url: CxMisc.finalizeUrl('/wygl/sfxt/dxcz/getSyts'),
			    type: "GET",
			    beforeSend: function(xhr, cfg) {
			    	$(`${cntrSelector} .toolbar .dx-remaining .loading`).show();
			    },
			    success: function(res, ts) {
			    	if (res.code == "0") {
			    		if (res.data) {
			    			$(`${cntrSelector} .toolbar .dx-remaining .number`).text(res.data);
			    		}
			    	} else {
			    ...
			    complete: function(xhr, ts) {
			    	$(`${cntrSelector} .toolbar .dx-remaining .loading`).hide();
			    }
			});
		},
