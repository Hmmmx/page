if (typeof window.KfBaxcBaxc === 'undefined') { // 可能多次载入运行，只在第一次时执行定义与赋值
	const cntrSelector = '#wgKfBaxcBaxcCntr';
	const dtlsModalSelector = '#wgKfBaxcBaxcModalDtls';
	const dtlsModalLblSelector = '#wgKfBaxcBaxcModalDtlsLabel';
	const dtlsFrmSelector = '#wgKfBaxcBaxcDtlsFrm';
	
	
	const mainTblClz = '.table-wgkf-baxc-baxc';
	
	const loadingMask1 = '#wgKfBaxcBaxcCntr';
	
	const me = window.KfBaxcBaxc = {
		pager: null,
		filterData: null,
		...
		bind: function() {
			...
			CxMisc.formValidated(dtlsFrmSelector, me.submitXcdd);
		...
		del: function(el, e) {
			...
			CxCtrl.confirm('是否确定删除此巡查地点？', function(src){
				CxMisc.ajax({
		            url: CxMisc.finalizeUrl('/wygl/kfxt/baxc/deleteXcdd'),
		            type: "GET",
		            data: {baxcid: id},
		            beforeSend: function(xhr, cfg) {
		            ...
		            success: function(res, ts) {
		            	if (res.code == "0") {
		            		CxMsg.info('删除巡查地点成功');
		            		me.refresh();
		            	} else CxMsg.error('删除巡查地点失败: ' + res.message);
		            },
		            error: function(xhr, ts, err) {
		            	var msg = "[" + xhr.status + " : " + ts + "]";
		            	CxMsg.error('删除巡查地点失败: ' + msg);
		            },
		            complete: function(xhr, ts) {
		...
		filter: function() {
			...
			if (me.filterData.sqdm) {
				CxMisc.ajax({
				    url: CxMisc.finalizeUrl('/wygl/kfxt/baxc/getXcddCount'),
				    type: "GET",
				    data: me.filterData,
				    beforeSend: function(xhr, cfg) {
				    	$(loadingMask1).mask('show');
		...
		goto: function(page, pageSize) {
			...
			CxMisc.ajax({
			    url: CxMisc.finalizeUrl('/wygl/kfxt/baxc/getXcdd'),
			    type: "GET",
		...
		render: function(page, pageSize, data) {
			...
			for (let i=0; i<data.length; i++) {
				let item = data[i];
				tmp.append(`<tr data-id="${item.baxcid}">
						<td class="td-indexer">
						...
						<td>${CxMisc.escapeHtml(item.xcddbt)}</td>
						...
						<td class="dl-item-cmd">
							...
							<a class="d-none" href="${CxMisc.finalizeUrl(item.ewmurl?item.ewmurl:'/resources/img/no-pic.png')}" download="${CxMisc.escapeHtml(item.xcddbt)}.png">download</a>
						</td>
					</tr>`);
		...
		openEdit: function (cmd, data) {
			...
			if (cmd == 'create') {
				f.baxcid.value = '';
				$(dtlsModalLblSelector).text('添加巡查地点');
			} else {
				f.baxcid.value = data.baxcid;
				f.xcddbt.value = data.xcddbt;
				if (data.bz) f.bz.value = data.bz;
				$(dtlsModalLblSelector).text('修改巡查地点');
			}
			$(dtlsModalSelector).modal('show');
		},

		
		submitXcdd: function(f) {
			...
			CxMisc.ajax({
			    url: CxMisc.finalizeUrl(data.cmd == 'create' ? '/wygl/kfxt/baxc/addXcdd' : '/wygl/kfxt/baxc/updateXcdd'),
			    type: "POST",
			    data: data,
			    beforeSend: function(xhr, cfg) {
