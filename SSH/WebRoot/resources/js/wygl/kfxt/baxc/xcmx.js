if (typeof window.KfBaxcXcmx === 'undefined') { // 可能多次载入运行，只在第一次时执行定义与赋值
	const cntrSelector = '#wgKfBaxcXcmxCntr';
	
	const mainTblClz = '.table-wgkf-baxc-xcmx';
	const filterFrmSelector = '#wgKfBaxcXcmxCntr .filterbar>form[data-type=filter]';
	
	
	const loadingMask1 = '#wgKfBaxcXcmxCntr';
	
	const me = window.KfBaxcXcmx = {
		pager: null,
		filterData: null,
		...
		filter: function(f) {
			...
			CxMisc.ajax({
			    url: CxMisc.finalizeUrl('/wygl/kfxt/baxc/getXcmxCount'),
			    type: "GET",
			    data: me.filterData,
			    beforeSend: function(xhr, cfg) {
		...
		goto: function(page, pageSize) {
			...
			CxMisc.ajax({
			    url: CxMisc.finalizeUrl('/wygl/kfxt/baxc/getXcmx'),
			    type: "GET",
		...
		render: function(page, pageSize, data) {
			...
			for (let i=0; i<data.length; i++) {
				let item = data[i];
				tmp.append(`<tr data-id="${item.baxcmxid}">
							<td class="td-indexer">
	    						<span>${page*pageSize + i+1}</span>
							</td>
							<td class="td-wrap">${CxMisc.escapeHtml(item.xcddbt)}</td>
				...
				if (imgs.length > 0) {
					...
					for (let j=0; j<imgs.length; j++) {
						thumbWrap.append(`<div class="col-12 d-i-flex-center thumbnail-group thumbnail-group-auto">
					                    <div style="height:80px;"><img class="c-pointer" src="${CxMisc.finalizeUrl(imgs[j])}" onload="KfBaxcXcmx.adjustImgSize(this)" style="max-width: none; max-height: 100%;"></div>
					                </div>`);
					}
				}
			}
