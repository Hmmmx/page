if (typeof window.gWgm === 'undefined') { // 可能多次载入运行，只在第一次时执行定义与赋值
	window.gWgm = {
		bindEvents: function() {
			$('#wgcdAddFuncMenu').click(function(){
				if ($('#WgMenuTree').data('currentNode'))
					gWgm.popSysMenuDialog('create', $('#WgMenuTree').data('currentNode').srcId);
				else
					CxMsg.info('请先从菜单树中选择功能菜单');
			});
			
			CxMisc.bindCheckAll('#wgcdCntr');
			
			$('#wgMenuDtlsFrm input[name=cdlx]').click(function(){
			...
			CxMisc.formValidated('#wgMenuDtlsFrm', gWgm.submitSysMenu);
			
			$('#wgModalMenuDtls').on('show.bs.modal', function (e) { CxMisc.clearValidation(this); }); //默认任何modal显示时把上次验证结果去掉
		},
			...
			if (parentNode.children) {
				...
				for (let i=0; i<parentNode.children.length; i++) {
					...
					if (node.cdlx == 2) {// !node.children || node.children.length==0
						...
						gWgm.convertNode2TreeNode(node, treeNode);
					}
		...
		deleteSysMenu: function(el, e) {
			let currentNode = $('#WgMenuTree').data('currentNode'), subNode = null,
			...
			CxCtrl.confirm('是否确定删除此功能菜单？', function(src){
				var target = $(src);
				CxMisc.ajax({
		            url: CxMisc.finalizeUrl('/wgcd/deleteXtgn'),
		            type: "GET",
		            data:{
		                cdid: cdid
		            },
		            beforeSend: function(xhr, cfg) {
		            	$('#wgcdCntr .col-extended').mask('show');
		            },
		            success: function(res, ts) {
		            	if (res.code == "0") {
		            		$('#WgMenuTree').treeview('disableNode', subNode.nodeId);
		            ...
		            complete: function(xhr, ts) {
		            	$('#wgcdCntr .col-extended').mask('hide');
		            	target.attr("disabled", true); // markAjaxEnd中去除了disabled，重新设置为disabled
		            }
		        });
		    }, {
		    	evt: evt,
		...
		fetchSubFuncMenu: function(node) {
			CxMisc.ajax({
			    url: CxMisc.finalizeUrl('/wgcd/findXtgn'),
			    type: "GET",
			    data: {cdid: node.srcId},
			    beforeSend: function(xhr, cfg) {
			    	$('#wgcdCntr .col-extended').mask('show', {msg: '载入中，请稍候...'});
			    },
			    success: function(res, ts) {
			    	if (res.code == "0") {
			    		if (res.data && res.data.length>0) {
			    			let tmp = $('#wgcdCntr .table-wg-menu tbody').empty();
			    			for (let i=0; i<res.data.length; i++) {
			    				tmp.append(`<tr data-id="${res.data[i].cdid}">
			    						<td class="td-indexer">
				    						<input type="checkbox" id="wgcdMenuIndexer-${i}" name="wgcdMenuIndexer${i}">
								            <label for="wgcdMenuIndexer-${i}" class="checkbox"><i class="far fa-square unchecked"></i><i class="far fa-check-square checked"></i></label>
				    						<span>${i+1}</span>
			    						</td>
			    						...
			    						<td class="dl-item-cmd">
			    							<div class="btn-group" role="group" aria-label="操作按纽组">
			    								<button type="button" class="btn btn-outline-primary" data-cmd="update" onclick="gWgm.popSysMenuDialog('update', '${res.data[i].cdid}', this)">修改</button>
			                                	<button type="button" class="btn btn-outline-danger" data-cmd="del" onclick="gWgm.deleteSysMenu(this)">删除</button>
			    							</div>
			    						</td>
			    					</tr>`);
			    			...
			    			$('#wgcdCntr .table-wg-menu tbody').empty().append('<tr><td colspan="7" class="table-empty">暂无数据</td></tr>');
			    		}
			    	} else {
		...
		loadFuncMenu: function() {
			CxMisc.ajax({
			    url: CxMisc.finalizeUrl('/wgcd/getXtgn'),
			    type: "GET",
			    beforeSend: function(xhr, cfg) {
			    	$('#wgcdCntr').mask('show');
			    },
			    success: function(res, ts) {
			    	if (res.code == "0" && res.data && res.data.length>0) {
			    		...
			    		gWgm.convertNode2TreeNode(root, treeRoot);
			    		gWgm.renderTreeView(treeRoot.nodes);
			    	} else {
		...
		popSysMenuDialog: function(cmd, menuId, el) {
			let f = document.querySelector('#wgMenuDtlsFrm');
			f.reset();
			f.cmd.value = cmd;
			if (cmd == 'create') {
				$('#wgModalMenuDtlsLabel').text('添加功能菜单');
				f.cdid.value = '';
				f.sjcdid.value = menuId;
				$(f).find("input[name=cdlx][value=1]").click(); // 恢复类型默认设置
			} else {
				$('#wgModalMenuDtlsLabel').text('修改功能菜单');
			...
			$('#wgModalMenuDtls').modal('show');
		},
		
		renderTreeView: function(data) {
			let tvCntr = $('#WgMenuTree'), currentNode = tvCntr.data('currentNode');
			tvCntr.treeview({
				data: data,
				...
				$('#WgMenuTree').data('currentNode', data);
				CxMisc.resetCheckAll('#wgcdCntr');
				gWgm.fetchSubFuncMenu(data);
			});
		...
		submitSysMenu: function(f) {
			let frm=$(f), prefix = f.cmd.value=='create' ? '添加' : '修改',
				url=f.cmd.value=='create' ? CxMisc.finalizeUrl('/wgcd/addXtgn') : CxMisc.finalizeUrl('/wgcd/updateXtgn');
			CxMisc.ajax({
			    url: url,
			    type: "POST",
